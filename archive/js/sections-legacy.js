document.addEventListener("DOMContentLoaded", () => {
	document.querySelectorAll(".section").forEach((section) => {
		...
		const imgHTML = img
			? `
        <div class="${
					type === "split50" ? "lrg-img-text-div-img" : "img-text-div-img"
				}">
      ...
      `
		...
		const textHTML = `
      <div class="${
				type === "split50" ? "lrg-img-text-div-text" : "img-text-div-text"
			}">
        ${section.innerHTML}
      </div>
    `;

		html = imgPos === "right" ? textHTML + imgHTML : imgHTML + textHTML;
